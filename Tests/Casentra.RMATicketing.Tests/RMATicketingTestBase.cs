...
namespace Casentra.RMATicketing.Tests
{
    public abstract class RMATicketingTestBase : AbpIntegratedTestBase<RMATicketingTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected RMATicketingTestBase()
        {
        ...
        protected void UsingDbContext(Action<RMATicketingDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<RMATicketingDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<RMATicketingDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<RMATicketingDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<RMATicketingDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<RMATicketingDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<RMATicketingDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<RMATicketingDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<RMATicketingDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<RMATicketingDbContext, Task<T>> func)
        {
            T result;
